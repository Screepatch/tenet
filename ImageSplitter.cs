...
class Program
{
    ...
    static int GetWorkMode()
    {
        ...
        do
        {
            Console.Write("Enter mode number (0, 1 or 2): ");
            string? input = Console.ReadLine();

            if (!string.IsNullOrEmpty(input) && int.TryParse(input, out mode) && mode is 0 or 1 or 2)
            {
                return mode;
            }
    ...
    static void SplitImage(Image originalImage, int rows, int columns, string outputDirectory)
    {
        ...
        for (int row = 0; row < rows; row++)
        {
            for (int col = 0; col < columns; col++)
            {
                ...
                Rectangle cropArea = new(x, y, currentCellWidth, currentCellHeight);

                // Создаем новое изображение для клетки
                using Bitmap cellBitmap = new(currentCellWidth, currentCellHeight);
    ...
    static void MergeImages(string[] imageFiles, int rows, int columns, string outputPath)
    {
        ...
        try
        {
            ...
            using Bitmap resultBitmap = new(totalWidth, totalHeight, pixelFormat);
                ...
                for (int row = 0; row < rows && imageIndex < imageFiles.Length; row++)
                {
                    for (int col = 0; col < columns && imageIndex < imageFiles.Length; col++)
                    {
                        ...
                        catch (OutOfMemoryException)
                        {
                            ...
                            FillEmptyCell(graphics, col * cellWidth, row * cellHeight, cellWidth, cellHeight, hasAlpha);
                        }
                        catch (FileNotFoundException)
                        {
    ...
    /// <summary>
    /// Заполняет пустую ячейку при ошибке загрузки изображения
    /// </summary>
    static void FillEmptyCell(Graphics graphics, int x, int y, int width, int height, bool hasAlpha)
    {
        using SolidBrush brush = new(hasAlpha ? Color.Transparent : Color.White);
